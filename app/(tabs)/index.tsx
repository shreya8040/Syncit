...
export default function HomeScreen() {
  ...
  return (
    ...
    <SafeAreaView style={{flex:1,alignItems: 'center', width:'100%',height:'100%', paddingTop:StatusBar.currentHeight ? StatusBar.currentHeight + 5 : 5, paddingLeft:5, paddingRight:10, backgroundColor:useThemeColor({},'background')}} edges={['left', 'right']}>
        ...
        <ImageBackground source={image1} resizeMode='cover' style={{flex:0.18, borderRadius:20 , overflow: 'hidden',  alignItems:'center', width:'100%', height:'100%'}} >
        ...
        {checklist.map((_, index) => (
          <Checkbox
            ...
            color={checkedStates[index] ? '#c0a596' :  undefined}
          />
        ))}
        
        
        </View>
        <Pressable style={  styles.pressableStyle1} onPress={() => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)}>
